...
// case-matched mutation on a Behavior. Middleware has no equivalent (it can't mutate state).

func emitOnReduce(matcher: String, payload: String, guarded: Bool) -> String {
    let isKeyPath = matcher == "keyPath"
    let isVoid = payload == "void"
    let generic = isVoid ? "" : "<T: Sendable>"
    // The `on` family's `\.case` spelling is the `@Prisms`-generated optional case property
    // (`KeyPath<Action, T?>`), matching the dispatch overloads in Behavior+Bridge.
    let firstParam = isKeyPath
        ? (isVoid ? "_ extract: KeyPath<Action, Void?>" : "_ extract: KeyPath<Action, T?>")
        : (isVoid ? "_ prism: Prism<Action, Void>" : "_ prism: Prism<Action, T>")
    let reduceParam = isVoid
        ? "reduce: @escaping @Sendable (inout State) -> Void"
        : "reduce: @escaping @Sendable (T, inout State) -> Void"
    let extractExpr = isKeyPath ? "action[keyPath: extract]" : "prism.preview(action)"

    var params = ["        \(firstParam),", "        \(reduceParam)\(guarded ? "," : "")"]
    if guarded { params.append("        when condition: @escaping @Sendable (State) -> Bool") }

    var lines: [String] = []
    let doc = "Matches \(isKeyPath ? "a key path" : "a `Prism`")\(isVoid ? " (Void case)" : "")"
        + " and runs a state mutation with no dispatch\(guarded ? ", guarded by a predicate" : "")."
    lines.append("    /// \(doc)")
    lines.append("    public func on\(generic)(")
    lines.append(contentsOf: params)
    lines.append("    ) -> Self {")
    let ctx = guarded ? "context" : "_"
    lines.append("        .combine(self, Behavior { action, \(ctx) in")
    if isVoid {
        lines.append("            guard \(extractExpr) != nil else { return .doNothing }")
    } else {
        lines.append("            guard let value = \(extractExpr) else { return .doNothing }")
    }
    if guarded {
        lines.append("            guard let state = context.stateBefore, condition(state) else { return .doNothing }")
    }
    lines.append("            return " + (isVoid ? ".reduce(reduce)" : ".reduce { state in reduce(value, &state) }"))
    lines.append("        })")
    lines.append("    }")
    return lines.joined(separator: "\n")
}

func generateBehaviorOnReduce() -> String {
    var variants: [String] = []
    for matcher in ["prism", "keyPath"] {
        for payload in ["value", "void"] {
            for guarded in [false, true] {
                variants.append(emitOnReduce(matcher: matcher, payload: payload, guarded: guarded))
            }
        }
    }
    return """
...
write(generateBehaviorOnReduce(), to: "__Generated__/Behavior+OnReduce.swift")
