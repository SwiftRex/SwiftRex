...
extension Effect where Dependencies == Void {
    public init<H: Hashable, P: CombineX.Publisher>(token: H, effect: P)
    where P.Output == DispatchedAction<OutputAction>, P.Failure == Never {
        self.token = token
        self._run = { _ in effect.eraseToAnyPublisher() }
    }

    public init<P: CombineX.Publisher>(_ effect: P) where P.Output == DispatchedAction<OutputAction>, P.Failure == Never {
        self.token = nil
        self._run = { _ in effect.eraseToAnyPublisher() }
    }
...
extension Effect {
    public static func fireAndForget<P: CombineX.Publisher>(_ upstream: @escaping (Context) -> P) -> Effect<Dependencies, OutputAction>
    ...
    public static func fireAndForget<P: CombineX.Publisher>(
...
extension Effect where Dependencies == Void {
    public static func fireAndForget<P: CombineX.Publisher>(_ upstream: P) -> Effect<Dependencies, OutputAction> where P.Failure == Never {
        Effect { _ in
            FireAndForget(upstream)
        }
    }

    public static func fireAndForget<P: CombineX.Publisher>(_ upstream: P, catchErrors: @escaping (P.Failure) -> DispatchedAction<OutputAction>?)
    -> Effect<Dependencies, OutputAction> {
        Effect { _ in
...
extension Effect {
    ...
    public static func sequence(_ values: OutputAction..., from dispatcher: ActionSource) -> Effect {
        Effect { _ in
            CombineX.Publishers.Sequence(sequence: values)
                .map { DispatchedAction($0, dispatcher: dispatcher) }
        }
    }
