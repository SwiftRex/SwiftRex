...
/// same `StateAccess` reference yields different values depending on *when* you access ``state``:
///
...
///
/// ```swift
/// Middleware<MyAction, MyState, MyEnvironment> { action, stateAccess in
///     let pre = stateAccess.state     // pre-mutation state (phase 1, @MainActor)
///
///     return Reader { env in
///         let post = stateAccess.state  // post-mutation state (phase 3, await elsewhere)
...
/// ## Store deallocation
///
/// ``state`` returns `nil` when the Store has been deallocated. Long-running effects
...
/// guard let current = stateAccess.state else {
...
public struct StateAccess<State: Sendable>: Sendable {
    ...
    /// The current state, or `nil` if the ``Store`` has been deallocated.
    ///
    /// Implicitly `@MainActor` (inherited from the struct). Synchronous when called from
    /// `@MainActor` context; use `await stateAccess.state` from any other async context.
    ///
    /// - Called during phase 1 (`Behavior.handle`): returns pre-mutation state.
    /// - Called during phase 3 (inside `Reader` / `produce` closures on `@MainActor`):
    ///   returns post-mutation state.
    public var state: State? { _get() }

    /// Package-internal snapshot — prefer ``state`` in new code.
    package func snapshotState() -> State? { _get() }
}
...
extension StateAccess {
    ...
    public func map<LocalState: Sendable>(
        ...
        StateAccess<LocalState> { self.state.map(f) }
    }
    ...
    public func flatMap<LocalState: Sendable>(
        ...
        StateAccess<LocalState> { self.state.flatMap(f) }
    }
}
