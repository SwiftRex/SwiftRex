/// ⛓ `MiddlewareProtocol` is a plugin, or a composition of several plugins, that are assigned to the `ReduxStoreProtocol` pipeline in order to
/// handle each action received (`InputActionType`), to execute side-effects in response, and eventually dispatch more actions (`OutputActionType`)
/// in the process.
///
...
/// A Store like `ReduxStoreProtocol` enqueues a new action that arrives and submits it to a pipeline of middlewares. So, in other words, a
/// `MiddlewareProtocol` is class that handles actions, and has the power to dispatch more actions to the `ActionHandler` chain. The `MiddlewareProtocol`
/// can also simply ignore the action, or it can execute side-effects in response, such as logging into file or over the network, or execute http
/// requests, for example. In case of those async tasks, when they complete the middleware can dispatch new actions containing a payload with the
/// response (a JSON file, an array of movies, credentials, etc). Other middlewares will handle that, or maybe even the same middleware in a future
/// RunLoop, or perhaps some `Reducer` will use that action to change the state.
///
/// Middlewares can read the action and the state before the reducer changed the app. At this point, you're in the main queue and you don't want to
/// block the queue, so avoid unnecessary work. In fact, avoid anything in there, including side-effects. To run more expensive tasks or be allowed to
/// dispatch actions back to the Store, you will be given an `AfterReducer` structure that allows you to schedule the side-effects you want to perform,
/// still being able to read the state (at this point, after it was changed by the reducer) and also dispatch new actions to the store. If you want to
/// compare the state before and after the reducer, all you have to do is caching the state before the reducer (when the action arrives), and reading
/// the state again inside the closure `AfterReducer.do`. You can use this technique to create Loggers comparing the state changes for each action,
/// audit changes, perform analytics tracking, telemetry or state sync with external devices, such as Apple Watches. Remote Debugging over the network
/// is also a great use of a Middleware.
///
...
///
/// Because the `MiddlewareProtocol` receive all actions and accesses the state of the app at any point, anything can be done from these small and
/// reusable boxes. For example, the same `CoreLocation` middleware could be used from an iOS app, its extensions, the Apple Watch extension or even
/// different apps, as long as they share some sub-state struct.
///
...
/// ```
///
/// `MiddlewareProtocol` is generic over 3 associated types:
///
/// #### InputActionType:
...
/// action of an app. Please check `lift(inputAction:outputAction:state:)` for more details.
///
/// #### OutputActionType:
/// The Action type that this `MiddlewareProtocol` will eventually trigger back to the store in response of side-effects. This can be the same as
/// `InputActionType` or different, in case you want to separate your enum in requests and responses.
/// Thanks to optics, this action can be a sub-action lifted to a global action type in order to compose with other middlewares acting on the global
/// action of an app. Please check `lift(inputAction:outputAction:state:)` for more details.
///
/// #### StateType:
/// The State part that this `MiddlewareProtocol` needs to read in order to make decisions. This middleware will be able to read the most up-to-date
...
/// Thanks to lenses, this state can be a sub-state lifted to a global state in order to compose with other middlewares acting on the global state of
/// an app. Please check `lift(inputAction:outputAction:state:)` for more details.
///
...
public protocol MiddlewareProtocol {
    ...
    /**
     ...
     of an app. Please check `lift(inputAction:outputAction:state:)` for more details.
     */
    ...
    ///                   You probably want to set the value of such as `afterReducer = .do { output in someOperation() }`, and the closure provided
    ///                   allows the `MiddlewareProtocol` to dispatch actions back to the store.
    func handle(
        action: InputActionType,
        from dispatcher: ActionSource,
        getState: @escaping GetState<StateType>,
        afterReducer: inout AfterReducer<OutputActionType>
    )
}
