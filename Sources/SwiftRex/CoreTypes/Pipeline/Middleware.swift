/**
 ...
 class APIMiddleware: Middleware {
     var getState: GetState<SomeStateType>!
     var output: AnyActionHandler<SomeActionType>!

     func receiveContext(getState: @escaping GetState<SomeStateType>, output: AnyActionHandler<SomeActionType>) {
         self.getState = getState
         self.output = output
     }

     func handle(action: SomeActionType, from dispatcher: ActionSource, afterReducer: inout AfterReducer) {
         guard action == .requestNetwork else { return }

         requestMyFavoriteAPI(completion: { [weak self] result in
             switch result {
             case let .success(value): self?.output.dispatch(.gotSuccessfulValue(value), info: "My Favorites API request callback")
             case let .failure(error): self?.output.dispatch(.gotFailure(error), info: "My Favorites API request callback")
             }
         })
     }
 }
 ...
 class LoggerMiddleware: Middleware {
     ...
     func handle(action: SomeActionType, from dispatcher: ActionSource, afterReducer: inout AfterReducer) {
         ...
         afterReducer = .do {
             ...
             self.log(action: action, from: source, before: stateBefore, after: stateAfter, dateBefore: dateBefore, dateAfter: dateAfter)
         }
     }
 }
 ```
