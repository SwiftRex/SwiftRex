...
/**
 ...
 Well, this unlocks several possibilities like having "View Stores", which act somehow like presenters (in MVP pattern)
 or ViewModels (in MVVM pattern), because this layer can be seen as a simple transformation of states and actions. The
 view store will transform State into View State by, for example, applying NumberFormatter or DateFormatter into numbers
 and dates from your state and generating strings to be shown in UI labels or text fields. You can picture that as
 dozens of small functions, transforming each property individually, or you can group all the properties in a view item,
 or view state, containing lots of strings to be used by UI controls, and then the whole process is a single function
 from `(State) -> ViewState`. In the other direction, you probably want to map UI events like scrolling, button taps,
 toggle changes and view lifecycle events (did load, will appear, foreground) into app actions like save a form, load
 next page, add to favorites or reload a list of items. Grouping them in an enum of possible events triggered by the
 user in a certain view, and you may want to map these view actions into app actions: `(ViewAction) -> AppAction`.
 ...
 Some store implementations will glue all the parts together and become responsible for being
 its responsibility is being a proxy to the non-Redux world. For that reason, it's correct to say that a `StoreType` is the single point of contact with `UIKit` and it's a class that you want to inject as a dependency on all the ViewControllers, either as one single dependency or, preferably, a dependency for each of its protocols - `EventHandler` and `StateProvider` -, both eventually pointing to the same instance.
