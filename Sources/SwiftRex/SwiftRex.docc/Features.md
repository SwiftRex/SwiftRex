...
`SwiftRex.Architecture` is the opinionated layer on top of `SwiftRex.SwiftUI`. Where <doc:BuildYourFirstFeature> wires a `Store`, a `Behavior`, and a view by hand, `@Feature` folds that wiring into a single `enum` namespace: you describe the feature, and the macro synthesizes `initialState(with:)` and an erased `view(store:environment:) -> some View`, applies `@Lenses` to `State` and `@Prisms` to `Action`, and builds the right kind of observable view store.

`@Feature(type:strategy:)` takes two knobs:

- **`type:`** — a `FeatureRole`. `.moduleEntryPoint` is a module's public entry (the generated `view()`/`initialState(with:)` are `public`, so the composing app can render and seed it); `.internalOnly` is a screen composed *inside* a module (the generated members stay `internal`).
...
| `typealias Content = SomeView` | to get a `view()` | logic-only feature, no `view()` |
...
A `.moduleEntryPoint` is the only thing a composing app sees of a module. Its `State`/`Action`/`Environment`/`Input` are `public` (they must be liftable), and so are the generated `view(store:environment:)` and `initialState(with:)`. It adds a seed (`Input`), an effect through the behavior, and state-driven navigation.
