...
```swift
// The wiring — declared once. An explicit action prism pins the parent's action type.
let detail = Relay.Scope.identity
    .action(AppAction.prism.detail).state(\AppState.detail).environment(\.detailEnv)
...
`Feature.view(store:environment:)` needs an environment, but a navigation destination runs inside the *environment-free* view body. A **router** — a value holding the store and the world — resolves that: its `@ViewBuilder view(for:)` switch builds each child (via `Relay.Scope`'s `.view(of:from:world:)` or directly), supplying the child's environment there. `some View` throughout — no `AnyView`.
