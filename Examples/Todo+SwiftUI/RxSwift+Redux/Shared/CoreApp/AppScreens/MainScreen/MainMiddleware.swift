...
let MainEffectMiddleware: SimpleEffectMiddleware<MainAction, MainState> = SimpleEffectMiddleware.onAction { action, actionSource, state in
  ...
  switch action {
      /// view action
  case .viewOnAppear:
    let publisher = Observable.just(MainAction.getTodo)
    return publisher
      .asEffect()
  case .toggleTodo(let todo):
    var todo = todo
    todo.isCompleted.toggle()
    let publisher = Observable.just(MainAction.updateTodo(todo))
    return publisher
      .asEffect()
  case .logout:
    let publisher = Observable.just(MainAction.changeRootScreen(.auth))
    return publisher
      .asEffect()
  case .viewCreateTodo:
    if state.title.isEmpty {
