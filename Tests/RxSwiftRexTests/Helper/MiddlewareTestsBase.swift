...
class MiddlewareTestsBase: XCTestCase {
    func lastActionInChain<A: ActionProtocol & Equatable>(_ action: A,
        ...
        -> (ActionProtocol, @escaping GetState<TestState>) -> Void { { chainAction, chainStateGetter in
    ...
    func lastEventInChain<E: EventProtocol & Equatable>(_ event: E,
                                                        state: TestState,
                                                        expectation: XCTestExpectation)
        -> (EventProtocol, @escaping GetState<TestState>) -> Void { { chainEvent, chainStateGetter in
